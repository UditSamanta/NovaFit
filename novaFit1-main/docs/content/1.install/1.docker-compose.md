...
This page provides instructions for installing and running NovaFIT using Docker Compose. This method is recommended for most users as it simplifies the setup process.
...
1.  **Clone the Repository**:
    If you haven't already, clone the NovaFIT repository to your local machine:
    ```bash
...
3.  **Start Docker Compose**:
    Navigate to the root directory of the `NovaFIT` project (where `docker-compose.prod.yml` is located) and run:
    ```bash
...
Once all services are up and running, you can access the NovaFIT frontend in your web browser at the URL you configured for `SPARKY_FITNESS_FRONTEND_URL` (e.g., `http://localhost:3004`).
