# Manual Backup and Restore for NovaFIT

This document provides instructions for performing manual backup and restore operations for NovaFIT, primarily for disaster recovery scenarios where the in-app restore might not be feasible.
...
**Location of Backups:**
All backup files are stored in a Docker volume mounted to the `NovaFITServer` service. The default path within the container is `/app/NovaFITServer/backup`. You will need to access this volume from your Docker host.

## 2. Manual Restore (Disaster Recovery)

This process is for situations where the NovaFIT application or its Docker containers are not functioning correctly, or if you need to restore to a completely new environment.
...
*   The `docker-compose.yml` file used to deploy NovaFIT.
...
1.  **Stop NovaFIT Services:**
    Navigate to the directory containing your `docker-compose.yml` file and stop the NovaFIT services.
...
2.  **Identify and Access the Backup Volume:**
    Find the Docker volume associated with the `NovaFITServer`'s backup directory. You can inspect your `docker-compose.yml` for volume definitions or use `docker volume ls` and `docker volume inspect <volume_name>`.
...
5.  **Clear Existing Database and Uploads Data:**
    ...
    *   **Clear `uploads` directory:**
        Identify the Docker volume for your `NovaFITServer`'s `uploads` directory (e.g., `sparkyfitness_server_uploads_data`).
        ```bash
...
7.  **Restore Uploads Directory:**
    Extract the `uploads` tar archive into the `NovaFITServer`'s `uploads` volume.
...
9.  **Start NovaFIT Services:**

    ```bash
    docker compose up -d
    ```

Your NovaFIT instance should now be restored to the state of your chosen backup.