...
This document covers the database structure, schema design, and migration process for NovaFIT.

## Database Overview

NovaFIT uses PostgreSQL 15+ with Row Level Security (RLS) to ensure data isolation between users. The database follows a normalized design with clear relationships between entities.
...
NovaFIT uses a custom migration system that runs automatically on server startup.
...
Migrations are stored in `NovaFITServer/db/migrations/` with the naming pattern:
```
...
1. **Create the migration file** in the migrations directory:
   ```bash
   cd NovaFITServer/db/migrations/
...
This database design provides a robust foundation for NovaFIT, ensuring data integrity, security, and performance while supporting all application features.
