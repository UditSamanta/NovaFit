...
const { configureSessionMiddleware } = require('../NovaFITServer'); // Import the session configuration function
...
async function performBackup(isManual = false) {
  ...
  const dbBackupFileName = `novafit_db_backup_${timestamp}.sql.gz`;
  const uploadsBackupFileName = `novafitness_uploads_backup_${timestamp}.tar.gz`;
  const fullBackupFileName = `novafitness_full_backup_${timestamp}.tar.gz`;
  ...
  try {
    ...
    const pgDumpArgs = [
      '-h', process.env.NOVA_FIT_DB_HOST,
      '-p', process.env.NOVA_FIT_DB_PORT,
      '-U', process.env.NOVA_FIT_DB_USER,
      '-d', process.env.NOVA_FIT_DB_NAME,
    ];
    const pgDump = spawn('pg_dump', pgDumpArgs, {
      env: { PGPASSWORD: process.env.NOVA_FIT_DB_PASSWORD, ...process.env },
    });
...
async function applyRetentionPolicy() {
  ...
  for (const file of files) {
    if (file.startsWith('novafitness_full_backup_') && file.endsWith('.tar.gz')) {
...
async function performRestore(backupFilePath, app) {
  ...
  try {
    ...
    const dbDumpFile = extractedFiles.find(f => f.startsWith('novafit_db_backup_') && f.endsWith('.sql.gz'));
    const uploadsTarFile = extractedFiles.find(f => f.startsWith('novafitness_uploads_backup_') && f.endsWith('.tar.gz'));
    ...
    const terminateConnectionsCommand = `SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '${process.env.NOVA_FIT_DB_NAME}' AND pid <> pg_backend_pid();`;
    await executeCommand(`psql -h ${process.env.NOVA_FIT_DB_HOST} -p ${process.env.NOVA_FIT_DB_PORT} -U ${process.env.NOVA_FIT_DB_USER} -d postgres -c "${terminateConnectionsCommand}"`, { env: { PGPASSWORD: process.env.NOVA_FIT_DB_PASSWORD, ...process.env } });
    log('info', 'Terminated active database connections.');

    // Drop and recreate database to ensure a clean state
    const dbEnv = { PGPASSWORD: process.env.NOVA_FIT_DB_PASSWORD, ...process.env };
    const dropDbCommand = `dropdb -h ${process.env.NOVA_FIT_DB_HOST} -p ${process.env.NOVA_FIT_DB_PORT} -U ${process.env.NOVA_FIT_DB_USER} ${process.env.NOVA_FIT_DB_NAME}`;
    const createDbCommand = `createdb -h ${process.env.NOVA_FIT_DB_HOST} -p ${process.env.NOVA_FIT_DB_PORT} -U ${process.env.NOVA_FIT_DB_USER} ${process.env.NOVA_FIT_DB_NAME}`;
    ...
    if (app) {
      const { configureSessionMiddleware } = require('../NovaFITServer');
