...
router.get('/nova-chat-history', authenticateToken, authorizeAccess('chat_history', (req) => req.userId), async (req, res, next) => {
  try {
...
router.get('/nova-chat-history/entry/:id', authenticateToken, authorizeAccess('chat_history'), async (req, res, next) => {
  const { id } = req.params;
  if (!id) {
...
router.put('/nova-chat-history/:id', authenticateToken, authorizeAccess('chat_history'), async (req, res, next) => {
  const { id } = req.params;
...
router.delete('/nova-chat-history/:id', authenticateToken, authorizeAccess('chat_history'), async (req, res, next) => {
  const { id } = req.params;
  if (!id) {
    return res.status(400).json({ error: 'Chat History Entry ID is required.' });
  }
