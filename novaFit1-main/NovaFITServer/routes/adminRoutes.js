...
// This will be enhanced later to prioritize NOVA_FIT_ADMIN_EMAIL
...
router.delete('/users/:userId', async (req, res, next) => {
  try {
    ...
    if (user.email === process.env.NOVA_FIT_ADMIN_EMAIL) {
...
router.put('/users/:userId/role', async (req, res, next) => {
  try {
    ...
    if (user.email === process.env.NOVA_FIT_ADMIN_EMAIL && role !== 'admin') {
