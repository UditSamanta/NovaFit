...
async function getChatHistoryByUserId(userId) {
  ...
  try {
    const result = await client.query(
      'SELECT content, message_type, created_at FROM nova_chat_history WHERE user_id = $1 ORDER BY created_at ASC LIMIT 5',
      [userId]
    );
    return result.rows;
  } finally {
...
async function getChatHistoryEntryById(id) {
  ...
  try {
    const result = await client.query(
      'SELECT * FROM nova_chat_history WHERE id = $1',
      [id]
    );
    return result.rows[0];
  } finally {
...
async function getChatHistoryEntryOwnerId(id) {
  ...
  try {
    const result = await client.query(
      'SELECT user_id FROM nova_chat_history WHERE id = $1',
      [id]
    );
...
async function deleteChatHistoryEntry(id, userId) {
  ...
  try {
    const result = await client.query(
      'DELETE FROM nova_chat_history WHERE id = $1 AND user_id = $2 RETURNING id',
      [id, userId]
    );
    return result.rowCount > 0;
  } finally {
...
async function clearAllChatHistory(userId) {
  ...
  try {
    await client.query(
      'DELETE FROM nova_chat_history WHERE user_id = $1',
      [userId]
    );
    return true;
  } finally {
...
async function saveChatHistory(historyData) {
  ...
  try {
    await client.query(
      `INSERT INTO nova_chat_history (user_id, content, message_type, metadata, created_at)
       VALUES ($1, $2, $3, $4, now())`,
