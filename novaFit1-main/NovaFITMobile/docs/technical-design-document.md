# NovaFIT Mobile App - Technical Design Document

## 1. Overview

This document outlines the technical design and architecture for the NovaFIT mobile application. The application will allow users to configure their server details, select health data to track (starting with steps), and sync this data with their personal NovaFIT server.
...
*   **API Client:** `axios` or the built-in `fetch` API for making REST API calls to the NovaFIT server.
...
graph TD
    ...
    J --> K[Send Data to NovaFIT Server];
...
The mobile app will communicate with the NovaFIT server via a REST API.
...
A new directory `NovaFITMobile` will be created.
