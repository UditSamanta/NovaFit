...
# Proxmox Deployment Script for NovaFIT

# This script automates the deployment of the NovaFIT application
...
# --- Environment Variables for NovaFIT (will be written to .env file) ---
# These are example values. You MUST change them for production.
NOVA_FIT_DB_NAME="sparkyfitness_db"
NOVA_FIT_DB_USER="sparkyuser"
NOVA_FIT_DB_PASSWORD="your_db_password" # CHANGE THIS!
NOVA_FIT_SERVER_PORT="3000"
NOVA_FIT_LOG_LEVEL="info"
NOVA_FIT_API_ENCRYPTION_KEY="your_api_encryption_key" # CHANGE THIS! Must be 32 characters long for AES-256
JWT_SECRET="your_jwt_secret" # CHANGE THIS!
NOVA_FIT_FRONTEND_URL="http://your_proxmox_ip:3004" # Change to your Proxmox host IP or domain
...
echo "--- Starting NovaFIT Proxmox Deployment ---"
...
services:
  ...
  sparkyfitness-server:
    ...
    ports:
      - "${NOVA_FIT_SERVER_PORT}:${NOVA_FIT_SERVER_PORT}" # Backend port
    environment:
      NOVA_FIT_SERVER_PORT: ${NOVA_FIT_SERVER_PORT}
      NOVA_FIT_LOG_LEVEL: ${NOVA_FIT_LOG_LEVEL}
      NOVA_FIT_DB_USER: ${NOVA_FIT_DB_USER}
      NOVA_FIT_DB_HOST: sparkyfitness-db # Use the service name 'sparkyfitness-db' for inter-container communication
...
if [ ! -f "./.env" ]; then
    ...
    echo ""
    echo "# NovaFIT Environment Variables"
    echo "# IMPORTANT: Change these values for production!"
    echo ""
    echo "NOVA_FIT_DB_NAME=\"$NOVA_FIT_DB_NAME\""
    echo "NOVA_FIT_DB_USER=\"$NOVA_FIT_DB_USER\""
    echo "NOVA_FIT_DB_PASSWORD=\"$NOVA_FIT_DB_PASSWORD\""
    echo "NOVA_FIT_SERVER_PORT=\"$NOVA_FIT_SERVER_PORT\""
    echo "NOVA_FIT_LOG_LEVEL=\"$NOVA_FIT_LOG_LEVEL\""
    echo "NOVA_FIT_API_ENCRYPTION_KEY=\"$NOVA_FIT_API_ENCRYPTION_KEY\""
    echo "JWT_SECRET=\"$JWT_SECRET\""
    echo "NOVA_FIT_FRONTEND_URL=\"$NOVA_FIT_FRONTEND_URL\""
    echo ""
...
echo "NovaFIT should now be running in LXC $LXC_ID."
echo "You can access the frontend at: $NOVA_FIT_FRONTEND_URL"
